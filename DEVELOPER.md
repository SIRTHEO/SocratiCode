...
**GDScript (Godot) support is conditional**: the `tree-sitter-gdscript` native addon is an optional dependency resolved via `node-gyp-build`. A preflight check in an isolated child process (`gdscript-preflight.cjs`) validates the addon's N-API compatibility, symbol exports, and ast-grep parse capability before registration. When the preflight passes, AST-based extraction avoids false matches in comments and strings. When it fails (e.g. missing prebuild for the platform, wrong ABI), a regex fallback handles preload/load/extends with comment-line skipping and triple-quoted-string stripping. Godot resource files (`.tscn`/`.tres`) use a tokenizer-based extractor that handles arbitrary whitespace, attribute order, and `uid://` paths. See the [TSCN file format docs](https://docs.godotengine.org/en/stable/contributing/development/file_formats/tscn.html) for the resource format specification.

### graph-resolution.ts

| Function | Signature | Description |
|----------|-----------|-------------|
| `resolveImport` | `(specifier, sourceFile, projectPath, fileSet, language, aliases?, jvmSuffixMap?, csNamespaceMap?, goModuleInfo?, phpPsr4Map?, dartPackageMap?, pythonImportRoots?, elixirModuleMap?, phpFqcnMap?, classNameIndex?, godotProjectRoot?, godotUidIndex?) → string \| null` | Resolve a module specifier to a project-relative file path; null for external/stdlib modules or a resolution miss |
...
| `buildGodotProjectIndexes` | `(projectPath, fileSet) → GodotProjectIndexes` | GDScript: per-project class_name indexes, keyed by Godot project root |
| `buildGodotUidIndexes` | `(projectPath, fileSet) → GodotProjectUidIndexes` | GDScript: per-project uid:// → relative path index, from .uid sidecars and .tscn/.tres headers |
...
| `findGodotRootForFile` | `(sourceFile, godotProjectIndexes?) → string \| null` | GDScript: find nearest project.godot ancestor for a single file |
