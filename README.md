...
| `BM25_TEXT` | `lexical` | What the BM25 (keyword) leg of hybrid search indexes for code collections. `lexical` (default) indexes the same text the dense vector embeds plus the words inside every multi-word identifier (`sendOperatorMessage` also as `send`, `Operator`, `Message`), and expands queries the same way, so a lookup by words meets the chunk that defines the identifier; `raw` indexes the embedded text alone — the behaviour of releases before this setting existed. The mode is part of a collection's effective index profile: an existing collection keeps its mode through incremental updates and is queried in that mode, and `codebase_status` lists `BM25_TEXT` among the requested changes that are inactive until the collection is freshly indexed. |
