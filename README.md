...
| `EMBEDDING_DOCUMENT_INCLUDE_PATH` | `true` | Whether the file path is embedded with the chunk, between `EMBEDDING_DOCUMENT_PREFIX` and the content. Accepts `true` / `1` / `yes` and `false` / `0` / `no`, case-insensitively and ignoring surrounding whitespace; leaving it empty is the same as leaving it unset, and any other value is rejected with an error naming it. Path tokens help path-shaped queries but add noise on prose-heavy corpora. Set to `false` to embed the document prefix and the chunk content only, with no separator between them other than whatever the prefix itself ends in. With `BM25_TEXT=raw` the same text feeds the dense embedding **and** the BM25 lexical index, so path-derived tokens stop matching in keyword search too; with the default `lexical` mode the BM25 text follows this setting as well. For context artifacts the `context:<name>:<path>` identifier is dropped along with the path. Existing collections keep their effective path setting; remove and freshly index the collection to activate a changed value. |
| `BM25_TEXT` | `lexical` | What the BM25 (keyword) leg of hybrid search indexes for code collections. `lexical` (default) indexes the file path, the code with comments removed, and the words inside every identifier (`sendOperatorMessage` also as `send`, `Operator`, `Message`), and expands queries the same way; `raw` indexes the same text the dense vector embeds, comments included — the behaviour of releases before this setting existed. Comments are removed through the parser for every language with an AST grammar; YAML, TOML, SQL and Dockerfiles use a small comment stripper; other documents are indexed whole. The mode is part of a collection's effective index profile: an existing collection keeps its mode through incremental updates and is queried in that mode, and `codebase_status` lists `BM25_TEXT` among the requested changes that are inactive until the collection is freshly indexed. |
